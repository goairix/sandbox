...
type ManagerConfig struct {
	...
	ActiveOperationTTL      time.Duration
	OperationRenewInterval  time.Duration
	RuntimeType             string
...
// Ready reports whether this replica can safely accept Kubernetes requests.
// Liveness intentionally remains independent so dependency outages do not
...
func (m *Manager) Ready(ctx context.Context) error {
	...
	if checker, ok := m.runtime.(interface{ Ping(context.Context) error }); ok {
...
func (m *Manager) Start(ctx context.Context) error {
	...
	if m.distributedStateEnabled() {
		if err := m.reconcileActiveLifecycleControllers(spanCtx); err != nil {
			return fmt.Errorf("reconcile active lifecycle controllers: %w", err)
		}
		m.wg.Add(1)
		go m.runActiveLifecycleCoordinator()
	}
...
// BeginShutdown removes this replica from readiness before the HTTP server is
// drained. It deliberately leaves controllers and request leases alive until
// Stop so in-flight requests remain fenced throughout graceful shutdown.
...
func (m *Manager) finishShutdown() {
	...
	for _, lifecycle := range syncLifecycles {
		...
		} else if lifecycle.ephemeralRecord != nil {
		...
		if lifecycle.controller != nil {
			shutdownErr = errors.Join(shutdownErr, lifecycle.controller.Stop(context.Background()))
		}
	}
	for _, lifecycle := range lifecycles {
		...
		} else if lifecycle.sandbox != nil && lifecycle.sandbox.Config.Mode == ModeEphemeral {
...
func (m *Manager) DrainRelease(ctx context.Context) error {
	...
	var drainErr error
	if m.distributedStateEnabled() {
		drainErr = errors.Join(drainErr, m.drainDistributedActiveSandboxes(ctx))
	}
...
func (m *Manager) Create(ctx context.Context, cfg SandboxConfig) (*Sandbox, error) {
	...
	if cfg.Network.Enabled || useBindMount || !ordinaryResourcesCompatible(cfg.Resources, m.pool.config) {
		source = "direct"
		spec := m.buildSpec(id, cfg)
		fillOrdinaryResourceDefaults(&spec, cfg.Resources, m.pool.config)
		if useBindMount {
	...
	if cfg.WorkspacePath != "" && !bindMounted {
		...
		controller, publishErr := m.publishActiveSandboxState(spanCtx, sb, lifecycle != nil)
		if publishErr != nil {
			_ = m.releasePreparedSyncWorkspace(lifecycle)
			if cfg.Mode == ModePersistent && m.sessions != nil {
			...
			return nil, fmt.Errorf("publish sandbox: %w", publishErr)
		}
...
func (m *Manager) createFUSESandbox(ctx context.Context, cfg SandboxConfig) (_ *Sandbox, returnErr error) {
	...
	stage, stageStarted := "prefix", time.Now()
	recordStage := func(result string) {
		spec := m.fusePool.spec.WorkspaceFUSE
		metrics.RecordWorkspaceStage(ctx, spec.RuntimeType, spec.Provider, stage, result, time.Since(stageStarted).Seconds())
	}
...
func (m *Manager) publishSandboxLifecycle(ctx context.Context, sb *Sandbox, gate *operationGate, lifecycle *fuseSandboxLifecycle, claim *fuseBindingClaim) error {
	...
	controller, publishErr := m.publishActiveSandboxState(ctx, sb, true)
	if publishErr != nil {
		cleanupCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), workspaceCleanupTimeout)
		defer cancel()
		return errors.Join(fmt.Errorf("publish active FUSE sandbox: %w", publishErr), m.removeWorkspaceLifecycle(cleanupCtx, sb, ephemeralRecord))
	}
	if controller != nil {
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-m.stopCh:
				if !m.distributedStateEnabled() && lifecycle.sandbox != nil && lifecycle.sandbox.Config.Mode == ModeEphemeral {
					m.runFUSETeardown(context.Background(), lifecycle, ErrSandboxNotReady)
				}
				return
			case <-ticker.C:
...
func (m *Manager) checkFUSELifecycle(ctx context.Context, lifecycle *fuseSandboxLifecycle) error {
	if m.distributedStateEnabled() {
		_, operationCtx, release, err := m.beginDistributedOperation(ctx, lifecycle.sandboxID, state.ActiveOperationData)
		if errors.Is(err, ErrSandboxNotReady) || errors.Is(err, state.ErrActiveSandboxAdmissionClosed) || errors.Is(err, state.ErrActiveSandboxConflict) {
			// Runtime inspection can also return NotReady. Only authoritative
			// closed admission proves that this health check should be skipped.
			record, loadErr := m.activeSandboxes.Load(ctx, lifecycle.sandboxID)
			if loadErr != nil {
				return errors.Join(err, loadErr)
			}
			if record != nil && (record.Phase == state.ActiveSandboxExclusive || record.Phase == state.ActiveSandboxDestroying || record.Phase == state.ActiveSandboxCleanupPending) {
				return nil
			}
		}
		if err != nil {
...
func (m *Manager) scheduleFUSETeardown(lifecycle *fuseSandboxLifecycle, cause error) {
	...
	lifecycle.teardownMu.Lock()
	if lifecycle.teardownScheduled || lifecycle.teardownDone {
		lifecycle.teardownMu.Unlock()
		m.lifecycleMu.Unlock()
		return
	}
	lifecycle.teardownScheduled = true
	lifecycle.teardownMu.Unlock()
	m.wg.Add(1)
	m.lifecycleMu.Unlock()
	go func() {
		...
		defer func() {
			lifecycle.teardownMu.Lock()
			lifecycle.teardownScheduled = false
			lifecycle.teardownMu.Unlock()
		}()
		ctx, cancel := context.WithTimeout(m.controlCtx, m.fuseTeardownTimeout())
		defer cancel()
		m.runFUSETeardown(ctx, lifecycle, cause)
	}()
}
...
func (m *Manager) Get(ctx context.Context, id string) (Sandbox, error) {
	...
	sb, _, release, err := m.acquireSandboxOperation(spanCtx, id)
	if err != nil {
		return Sandbox{}, err
	}
	defer release()
...
func cloneSandbox(sb *Sandbox) Sandbox {
	...
	clone.WorkspaceTransitionExclude = append([]string(nil), sb.WorkspaceTransitionExclude...)
	if sb.Workspace != nil {
...
func (m *Manager) acquireSandboxOperation(ctx context.Context, id string) (*Sandbox, context.Context, func(), error) {
	if m.distributedStateEnabled() {
	...
	if ok {
		if gate != nil {
			...
			if err != nil {
				return nil, nil, nil, fmt.Errorf("%w: %s", err, id)
			}
			m.mu.RLock()
			...
			m.mu.RUnlock()
			return current, ctx, release, nil
		}
...
func (m *Manager) Exec(ctx context.Context, id string, req runtime.ExecRequest) (*runtime.ExecResult, error) {
	...
	sb, operationCtx, release, err := m.acquireSandboxOperation(execCtx, id)
	if err != nil {
		telemetry.Error(err, span)
...
func (m *Manager) UploadFile(ctx context.Context, id, destPath string, size int64, reader io.Reader) error {
	...
	sb, operationCtx, release, err := m.acquireSandboxOperation(ctx, id)
	if err != nil {
		return err
	}
	ctx = operationCtx
...
func (m *Manager) UpdateNetwork(ctx context.Context, id string, enabled bool, whitelist []string, blockPrivate bool) error {
	...
	sb, operationCtx, release, err := m.acquireSandboxMutation(ctx, id)
	if err != nil {
		return err
	}
	ctx = operationCtx
...
func (m *Manager) autoSyncOnce() {
	if m.distributedStateEnabled() {
		m.autoSyncDistributedWorkspaces()
		return
	}
	...
	for _, t := range targets {
		sb, operationCtx, release, acquireErr := m.acquireSandboxOperation(ctx, t.sandboxID)
		if acquireErr != nil {
		...
		syncErr := m.syncFromContainer(operationCtx, t.sandboxID, runtimeID, t.syncExclude)
		release()
		if syncErr != nil {
...
func (m *Manager) restoreFUSESandbox(ctx context.Context, sb *Sandbox) error {
	return m.restoreFUSESandboxWithController(ctx, sb, nil)
}

func (m *Manager) restoreFUSESandboxWithController(ctx context.Context, sb *Sandbox, controller *activeController) error {
	return m.restoreFUSELifecycleWithController(ctx, sb, controller, false)
}

// Cleanup reconstruction never publishes ready admission. A live controller,
// the durable cleanup phase, exact consumed pool record, Pod UID and owner
// still fence every subsequent teardown operation. Quiesced health is not a
// reason to abandon that evidence after the requesting process has crashed.
func (m *Manager) restoreFUSECleanupWithController(ctx context.Context, sb *Sandbox, controller *activeController) error {
	if controller == nil || sb == nil || !m.distributedStateEnabled() {
		return ErrSandboxNotReady
	}
	...
	if record == nil || (record.Phase != state.ActiveSandboxDestroying && record.Phase != state.ActiveSandboxCleanupPending) ||
		record.Generation != sb.activeGeneration || record.RuntimeID != sb.RuntimeID || record.RuntimeUID != sb.RuntimeUID {
		return ErrSandboxNotReady
	}
	if strings.HasPrefix(record.CleanupCheckpoint, fuseCleanupCheckpointPrefix) {
		return m.restoreCheckpointedFUSECleanup(ctx, sb, controller, record)
	}
	return m.restoreFUSELifecycleWithController(ctx, sb, controller, true)
}

func (m *Manager) restoreFUSELifecycleWithController(ctx context.Context, sb *Sandbox, controller *activeController, cleanup bool) error {
	if sb == nil || sb.Workspace == nil || m.fusePool == nil ||
	...
	if sb.ID == "" || (sb.Config.Mode != ModePersistent && sb.Config.Mode != ModeEphemeral) || sb.State != StateReady ||
	...
	if !cleanup && (err != nil || health == nil || !health.Ready || health.MountType != "fuse" ||
		health.RuntimeUID != sb.RuntimeUID || health.Generation != owner.Generation ||
		health.RestartCount != 0 || health.RestartDetected) {
		return ErrSandboxNotReady
	}
	if cleanup && health != nil && (health.MountType != "fuse" || health.RuntimeUID != sb.RuntimeUID || health.Generation != owner.Generation) {
		return ErrSandboxNotReady
	}
	...
	restoreMu.Lock()
	var ephemeralRecord *EphemeralLifecycleRecord
	if sb.Config.Mode == ModeEphemeral {
		ephemeralRecord, err = m.ephemeral.Load(ctx, sb.ID)
		if err != nil {
			restoreMu.Unlock()
			cancelLifecycle()
			renewal.Stop()
			return fmt.Errorf("load ephemeral FUSE lifecycle: %w", err)
		}
	}
	lifecycle = &fuseSandboxLifecycle{
		sandboxID:       sb.ID,
...
// drainOrphanedOrdinaryPoolContainers removes Kubernetes ordinary warm-pool
// runtimes that are not represented in this Manager's in-memory Pool.
// Release-drain callers have already stopped every API replica and finalized
// user sandboxes, so any remaining non-FUSE runtime carrying sandbox.pool=true
// is orphaned. Unlike the startup cleanup, this path is fail-closed because
// Helm must not proceed while managed resources remain in the runtime namespace.
func (m *Manager) drainOrphanedOrdinaryPoolContainers(ctx context.Context) error {
	...
	for _, container := range containers {
		if container.Labels["sandbox.workspace.mode"] == string(WorkspaceMountFUSE) ||
			container.Labels["sandbox.role"] == "fuse-runtime" {
			continue
		}
...
func (m *Manager) GetMultipartStatus(ctx context.Context, sandboxID, uploadID string) (*MultipartUploadState, error) {
	_, operationCtx, release, err := m.acquireSandboxOperation(ctx, sandboxID)
	if err != nil {
		return nil, err
	}
	ctx = operationCtx
