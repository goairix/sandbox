...
runtime_installer="$repo_root/docker/images/sandbox/install-runtime.sh"
s3fs_builder="$repo_root/docker/images/common/build-s3fs.sh"
...
grep -Fq 'SANDBOX_STORAGE_FILESYSTEM_ACCESS_KEY=${STORAGE_ACCESS_KEY:-}' "$compose" || fail "Compose omits the inline access key mapping"
grep -Fq 'SANDBOX_STORAGE_FILESYSTEM_SECRET_KEY=${STORAGE_SECRET_KEY:-}' "$compose" || fail "Compose omits the inline secret key mapping"
...
for file in "$mounter" "$fuse"; do
  ...
  grep -Fq 'S3FS_COMMIT=561ce1e20600bab42133b5e51511483800d3ef76' "$file" || fail "$file does not pin the verified s3fs 1.95 commit"
  grep -Fq 'COPY docker/images/common/build-s3fs.sh' "$file" || fail "$file does not use the repository s3fs source builder"
  test "$(grep -Fc 'ARG DEBIAN_MIRROR=http://mirrors.aliyun.com/debian' "$file")" -eq 2 || fail "$file does not default both Debian stages to the domestic mirror"
  test "$(grep -Fc 'ARG DEBIAN_SECURITY_MIRROR=http://mirrors.aliyun.com/debian-security' "$file")" -eq 2 || fail "$file does not default both Debian stages to the domestic security mirror"
  test "$(grep -Fc 's|http://deb.debian.org/debian-security|${DEBIAN_SECURITY_MIRROR}|g' "$file")" -eq 2 || fail "$file does not configure both Debian security mirrors"
  grep -Fq 'libfuse-dev' "$file" || fail "$file does not build s3fs against its supported FUSE 2 API"
  ! grep -Fq 'libfuse3-dev' "$file" || fail "$file incorrectly builds s3fs 1.95 against FUSE 3"
  grep -Eq 'ca-certificates fuse3 libcurl4 libfuse2 libssl3 libxml2' "$file" || fail "$file omits the FUSE 2 runtime library or FUSE 3 helper"
  test "$(grep -Fc 'apt-get -o Acquire::Retries=5' "$file")" -ge 4 || fail "$file does not retry transient package downloads"
  test "$(grep -Fc 'Acquire::http::Pipeline-Depth=0' "$file")" -ge 4 || fail "$file does not tolerate truncating HTTP proxies"
  ...
  ! grep -Eq '^ARG (BASE_IMAGE|S3FS_PACKAGE_URL|S3FS_PACKAGE_SHA256)(=|$)' "$file" || fail "$file still requires an operator-supplied FUSE build input"
  grep -Fq 'sha256sum /usr/bin/s3fs' "$file" || fail "$file does not record the installed s3fs binary hash"
...
grep -Fq 'FROM python:3.13-slim-bookworm' "$fuse" || fail "special image does not use its concrete sandbox base"
grep -Fq 'COPY docker/images/sandbox/install-runtime.sh' "$fuse" || fail "special image does not share the sandbox runtime installer"
...
grep -Fq 'FROM python:3.13-slim-bookworm' "$ordinary" || fail "ordinary image does not use its concrete sandbox base"
grep -Fq 'COPY docker/images/sandbox/install-runtime.sh' "$ordinary" || fail "ordinary image does not use the shared runtime installer"
grep -Fq 'groupadd -g 1000 sandbox' "$runtime_installer" || fail "shared runtime installer does not pin the sandbox GID"
grep -Fq 'useradd -g sandbox -u 1000' "$runtime_installer" || fail "shared runtime installer does not pin the sandbox UID"
grep -Fq 'git checkout -- configure.ac' "$s3fs_builder" || fail "s3fs source build leaves the pinned source tree dirty after autogen"
...
grep -Fxq '!docker/images/sandbox/install-runtime.sh' "$ordinary_ignore" || fail "ordinary build context omits the shared runtime installer"
...
"$repo_root/scripts/test-inline-credential-config.sh"
