...
API 套件另外三次 FUSE 创建中位数 8.843 秒、最大 11.424 秒，其间有工作区矩阵并行。这些均为小样本耗时，不是长期 p95/p99 或公平独占性能对比。普通销毁仍等待正常终止完成；本批解决竞争 503，不宣称已消除该等待。后续只读排查确认普通 PID 1 为未注册退出信号处理器的 `sleep infinity`，30 秒是宽限上限而非必须等待；FUSE 还存在 Pod Ready 探测链路等待，尚无逐请求阶段占比。两项性能优化及其它未完成事项已记录在 [Kubernetes 后续事项](2026-09-13-kubernetes-follow-ups.md)，按用户要求后续再处理。
