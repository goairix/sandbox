...
Destroy permanently closes the operation gate and waits for all stream references, stops lease renewal, runs the sandbox-side quiesce probe, performs bounded best-effort flush only when quiesce succeeded, atomically claims the Pool record as cleanup, removes the Pod/container with matching termination/fencing evidence, then compare-and-deletes lease/owner and deletes the exact cleanup tombstone before refill. If exit cannot be confirmed, retain owner, record and policies/resources and mark blocked. A failed flush is reported as not durable but never upgraded to `flushed=true`.
