...
`NewFUSEPoolRepository(store *Store)` is the only concrete repository constructor; `Store` itself is not asserted to implement this domain interface and its Redis client remains private. `PreparationID` is an opaque, bounded, valid-UTF-8 UUID generated before runtime allocation and Redis keys contain only its digest. Implement admission, bind, reserve, revision-aware transitions, cleanup claim/delete, inventory and refill-lock operations with Lua so state/token/revision checks and mutations are atomic. Admission validates the held refill token and `preparing + prepared < max_size` in the same Lua; bind makes RuntimeUID immutable and globally unique; publish validates the same lock token. Returning `reserved → prepared` performs the same `preparing + prepared < max_size` admission in its transition Lua. `PrepareUntil`, `ReservedUntil` and `CleanupUntil` come from Redis `TIME`, not API wall clocks. Refill locks are token-renewed and every fenced write still rejects a lost token. Cleanup claims are idempotent for the same token, reject another token before expiry, and permit stale takeover after expiry. If runtime identity becomes known only after an intent-only cleanup claim, the same-token retry atomically validates and fills both the record UID mapping and the global RuntimeUID owner index; a conflicting mapping is corruption/conflict, never an overwrite. Runtime deletion occurs only after a successful exact cleanup claim; a failed removal keeps the cleanup tombstone retryable. Physical record removal is available only through `DeleteCleanup`; there is no direct deletion API for live states. Every successful non-idempotent mutation increments `Revision`; stale reconcilers leave the newer record and runtime untouched. Never auto-transition an expired reservation to prepared.
...
Build `workspace-probe` into every FUSE-capable sandbox image. Its fixed `write-read-delete` command runs as UID/GID 1000. The probe object basename is the versioned, domain-separated SHA-256 result of exact RuntimeUID plus lease generation; it is reproducible without exposing the raw UID and is never supplied by a public request. Its `quiesce` command runs inside the sandbox container, stops all other same-UID descendants, verifies no writable file descriptor targets `/workspace`, and emits a bounded token used to resume them when a flush does not destroy the sandbox. Failure to enumerate, stop or verify any process is an error; the Manager must report `flushed=false` and may not call the mounter flush path.
...
Task 14 reserved-key contract: FUSE public file list/stat/read/write/remove paths must hide or reject the single basename shape recognized by `fuseprotocol.IsReservedProbeObjectName`; clients can never select it as a business object. This rule applies only to the exact versioned reserved basename and must never become a prefix/glob deletion rule.
...
Restore delivered FUSE sandboxes only when session, owner, runtime UID and generation agree and `WorkspaceHealth` is ready; only then install an open operation gate and lifecycle watcher, and never call authorize for `mount_attempt=1`. A direct SessionStore load does not open the gate. Reconcile Pool records before WarmUp: reuse prepared only after pristine probe; classify timed-out preparing/reserved and binding records through the persistent owner/session/gate guard; claim cleanup only for explicit Pristine/Abandoned states (binding only Abandoned); and retain Protected/Unknown/error records as unavailable. Consumed remains owned by its persisted session. Build the protected runtime UID set from valid sessions, owners and pool records, then invoke the optional `OrphanReconciler`; this is the first point at which Docker gateways/networks may be cleaned.

Destroy permanently closes the operation gate and waits for all stream references, stops lease renewal, runs the sandbox-side quiesce probe, performs bounded best-effort flush only when quiesce succeeded, atomically claims the Pool record as cleanup, and removes the Pod/container with matching termination/fencing evidence. After that evidence, Task 15 must use the Manager-owned `WorkspaceObjectClient` plus the canonical workspace prefix to delete exactly `fuseprotocol.DeriveProbeObjectName(RuntimeUID, generation)` and verify its absence; it must not list or wildcard-delete probe-looking/user objects. Only then compare-and-delete lease/owner and delete the exact cleanup tombstone before refill. If exact reserved-key cleanup cannot be confirmed, retain the cleanup tombstone for recovery and fail closed. If exit cannot be confirmed, retain owner, record and policies/resources and mark blocked. A failed flush is reported as not durable but never upgraded to `flushed=true`.
